...
//!
//! Measures the zero-allocation pipeline: JSON decode → float parse → merge.
//!
//! Run: `cargo bench`
...
use orderbook_aggregator::types::{Level, OrderBook};
...
fn make_book(exchange: &'static str, base_bid: f64, base_ask: f64, n: usize) -> OrderBook {
    OrderBook {
        ...
        bids: (0..n)
            .map(|i| Level {
                exchange,
                price: base_bid - i as f64 * 0.001,
                amount: 10.0 - (i % 8) as f64,
            })
            .collect(),
        asks: (0..n)
            .map(|i| Level {
                exchange,
                price: base_ask + i as f64 * 0.001,
                amount: 8.0 - (i % 6) as f64,
            })
            .collect(),
...
fn bench_binance_decode(c: &mut Criterion) {
    ...
    c.bench_function("binance_decode_20", |b| {
        b.iter_batched(
            || json.clone(),
            |mut s| black_box(parse_depth_json(&mut s)),
...
fn bench_bitstamp_decode(c: &mut Criterion) {
    ...
    c.bench_function("bitstamp_decode_20", |b| {
        b.iter_batched(
            || json.clone(),
            |mut s| black_box(parse_order_book_json(&mut s)),
            criterion::BatchSize::SmallInput,
        );
    });
}
...
fn bench_float_parse(c: &mut Criterion) {
    c.bench_function("fast_float_parse", |b| {
        b.iter(|| {
            black_box(fast_float::parse::<f64, _>("0.06824000").unwrap());
            black_box(fast_float::parse::<f64, _>("12.50000000").unwrap());
        });
    });
}
...
fn bench_e2e(c: &mut Criterion) {
    ...
    c.bench_function("e2e_parse_merge", |b| {
        b.iter_batched(
            || (binance.clone(), bitstamp.clone()),
            |(mut bin_json, mut bts_json)| {
                let book_a = parse_depth_json(&mut bin_json).unwrap();
                let book_b = parse_order_book_json(&mut bts_json).unwrap();
                let mut store = BookStore::new();
                store.insert(book_a);
                store.insert(book_b);
